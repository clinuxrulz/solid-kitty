...
export class World {
    ...
    update(params: {
        ...
        }
        {
            ...
            let startI = Math.floor(
                this.state.camera.pos.y / RENDER_BLOCK_HEIGHT,
            );
            let startJ = Math.floor(
                this.state.camera.pos.x / RENDER_BLOCK_WIDTH,
            );
            let numRows = Math.ceil(
                params.windowSize.height / RENDER_BLOCK_WIDTH,
            );
            let numCols = Math.ceil(
                params.windowSize.width / RENDER_BLOCK_HEIGHT,
            );
                ...
                for (let j = startJ; j < startJ + numCols; ++j) {
                    ...
                    if (monsterConstructor2 != undefined) {
                        ...
                        newActor.actor.setState("pos", {
                            ...
                            y:
                                (i + 1) * RENDER_BLOCK_HEIGHT -
                                newActor.actor.state.size.y,
                        });
        ...
        }
        {
            ...
            // Unless it is kitty.
            let minXIdx = Math.floor(
                this.state.camera.pos.x / RENDER_BLOCK_WIDTH,
            );
            let minYIdx = Math.floor(
                this.state.camera.pos.y / RENDER_BLOCK_HEIGHT,
            );
            let numRows = Math.ceil(
                params.windowSize.height / RENDER_BLOCK_WIDTH,
            ...
            for (let i = actors.length - 1; i >= 0; --i) {
                ...
                let actorXIdx = Math.ceil(
                    actor.actor.state.pos.x / RENDER_BLOCK_WIDTH,
                );
            ...
            actor.actor.setState("pos", "y", y);
            {
                ...
                for (let yIdx = minYIdx; yIdx <= maxYIdx; ++yIdx) {
                    for (let xIdx = minXIdx; xIdx <= maxXIdx; ++xIdx) {
                        ...
                        let leftCell = this.state.level.readBlock(
                            xIdx - 1,
                            yIdx,
                        );
                        let rightCell = this.state.level.readBlock(
                            xIdx + 1,
                            yIdx,
                        );
                        let topCell = this.state.level.readBlock(
                            xIdx,
                            yIdx - 1,
                        );
                        let bottomCell = this.state.level.readBlock(
                            xIdx,
                            yIdx + 1,
                        );
                        if (cell == "G") {
