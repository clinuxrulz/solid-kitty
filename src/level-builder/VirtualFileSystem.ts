...
export class VirtualFileSystem {
        ...
        if (filesAndFolders3.length != 0) {
            let r = await new Promise<Result<{}>>((resolve) => {
                let cursor = this.db
                    ...
                    .put(
                        filesAndFolders3,
                        parentFolderId2 + "_files_and_folders",
                    );
                cursor.onerror = () => {
        ...
        }
        {
            let r = await new Promise<Result<{}>>((resolve) => {
                let cursor = this.db
                    ...
                    .put(
                        filesAndFolders3,
                        parentFolderId + "_files_and_folders",
                    );
                cursor.onerror = () => {
                    resolve(err("Could not add file."));
                };
    ...
    async deleteFolder(folderId: string): Promise<Result<{}>> {
        ...
        let filesAndFolders2 = filesAndFolders.value;
        let filesAndFolders3 = filesAndFolders2.filter(
            (x) => x.id !== folderId,
        );
        if (filesAndFolders3.length != 0) {
            let r = await new Promise<Result<{}>>((resolve) => {
    ...
    async delete(fileOrFolderId: string) {
        ...
        let filesAndFolders2 = filesAndFolders.value;
        let fileOrFolder = filesAndFolders2.find(
            (x) => x.id === fileOrFolderId,
        );
