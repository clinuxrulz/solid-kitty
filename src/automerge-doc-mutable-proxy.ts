    ...
    [undefined] as T,
    {
          ...
          return function(predicate: (value: any, index: number, obj: any[]) => boolean, thisArg?: any) {
            ...
            for (let i = 0; i < json.length; i++) {
              if ((elementSchema as any).type == "Object") {
                tempArray.push(projectMutableOverAutomergeDoc(
                  json[i],
                  (cb) => updateJson((json2) => cb(json2[i])),
                  elementSchema as any,
                ));
              } else if ((elementSchema as any).type == "Array") {
                tempArray.push(projectMutableOverAutomergeDocArray(
                  json[i],
                  (cb) => updateJson((json2) => cb(json2[i])),
                  elementSchema as any,
                ));
      ...
      set(target, p, newValue, receiver) {
        ...
        if (p === "length") {
          ...
          if (n > 0) {
            let x = Array(n).fill(undefined).map((_) => makeDefaultViaTypeSchema(elementSchema as any));
