...
export class Node {
    ...
    Render: Component<{ isHighlighted: boolean, isSelected: boolean, }> = (props) => {
        return (<>
            <circle
            ...
            <For each={this.state.sinks}>
                {(sink) => {
                    ...
                    let verts = createMemo(() => {
                        let u = sink.state.position.clone().sub(this.state.position).normalize();
                        if (!Number.isFinite(u.x)) {
                            return undefined;
                        }
                        let v1 = u.clone().multScalar(NODE_RADIUS).add(this.state.position);
                        let v2 = u.clone().multScalar(-NODE_RADIUS).add(sink.state.position);
                        return { v1, v2 };
                    });
                    return (
                        <Show when={verts()}>
